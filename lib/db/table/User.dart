import 'package:flutter_gionchat/db/helper/SQLiteHelper.dart';
...
class User {
  ...
  Future<int> insert<T extends User>() async {
    Database?  database = await SQLiteHelper.getInstance().openDb(SQLiteHelper.DB_NAME);
  ...
  // 查询数据
  Future<List<User>> query<User>(
      {String key = "", String value = ""}) async {
    Database?  database = await SQLiteHelper.getInstance().openDb(SQLiteHelper.DB_NAME);
    if (null == database || !database.isOpen) return [];
    List<Map<String, dynamic>> maps = List.filled(0, Map<String, dynamic>());
